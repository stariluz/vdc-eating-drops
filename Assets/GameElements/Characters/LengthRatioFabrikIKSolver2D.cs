...
namespace UnityEngine.U2D.IK
{
    ...
    // [IconAttribute(IconUtility.IconPath + "Animation.ProportionalFabrikIcon.png")]
    public sealed class LengthRatioFabrikIKSolver2D : Solver2D
    {
        ...
        protected override void DoUpdateIK(List<Vector3> targetPositions)
        {
            ...
            bool result=LENGTH_RATIO_FABRIK_2D.Solve(m_TotalLength, originalPostion, targetPosition, iterations, tolerance, m_Lengths, m_LengthRatios, ref m_Positions);
    ...
    {
        public static bool Solve(float strechThreshold, Vector2 originalTargetPosition, Vector2 targetPosition, int solverLimit, float tolerance, float[] lengths, float[] length_ratios, ref Vector2[] positions)
        {
            int length = positions.Length - 1;
            int iteration = 0;
            float sqrTolerance = tolerance * tolerance;
            Vector2 strechDiff = targetPosition - originalTargetPosition;
        ...
        {
            int positionsLength = positions.Length - 1;
            positions[positionsLength] = targetPosition;
            for (int index = positionsLength - 1; index >= 0; index--)
            {
